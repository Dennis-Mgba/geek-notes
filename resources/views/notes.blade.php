    ...
    <section class="notes">
        ...
        @for ($i=0; $i < count($notes); $i++)
            <article class="note">    {{--using tenary {{ $i % 3 === 0 ? ' first-in-line' : (($i + 1) % 3 === 0 ? ' last-in-line' : '') }} --}}
                ...
                <div class="info">Created by <a href="#">{{ $notes[$i]->author->author_name }}</a> {{ $notes[$i]->created_at }} </div> {{--the ->author is the function relationship declared in the note model while author_name is the name of the table in the authors table--}}
            </article>
        @endfor
        <div class="">
            pagination
